 ...
 * 
 */
public class LibSVMPredictor extends ALearner {
	String cmdLine;
	String fn_trainSet;
	String fn_testSet;
	String fn_model;
	...
	public String initialTrainingParameter(String svmLightPath,
		...
		if (svmLightPath.equals("")) {
			...
			// "svm-train"}) + "\"";
			cmdLine = Util
					.getOSPath(new String[] { svmLightPath, "svm-train" });
		}

		// SVMLight control param
		cmdLine += " -q"; // no output
		cmdLine += " -c 1"; // C = 1 => instance's final Cost = C * instance's
							// Cost (get from train set)
		cmdLine += " -t 0"; // linear kernel
	...
	public String initialTestingParamter(String svmLightPath,
		...
		if (svmLightPath.equals("")) {
			...
			// "svm-predict"}) + "\"";
			cmdLine = Util
					.getOSPath(new String[] { svmLightPath, "svm-predict" });
		}
	...
	protected double[][] convertLibSVMPredictionIntoPredictionMatrix(
		...
		String filename = "/ext_data/SVM_preds.csv";
	 ...
	 *            The test set file (has SVMLight file format)
	...
	/**
	 * Get feature weights from the model saved in <code>fn_model</code>. The
	 * feature weight order is the feature order.
	 * <p>
	...
	public FeatureWeight[] getFeatureWeights(String fn_model, boolean sorted)
		...
		String[] lines = Util.loadList(fn_model);
		FeatureWeight[] featureWeight = null;

		if (!lines[1].contains("linear")) {
			System.out.println("This SVM does not have linear kernel!");
		}
		// else if(! lines[10].contains("threshold b")) {
		// System.out.println("Parsing error!");
		// }
		else {
			String features, feat;
			double alpha;
			int index;
			String[] featValPair;
			for (int lineCount = 10; lineCount < lines.length; lineCount++) {
				features = lines[lineCount];
				// comments =
				// lines[lineCount].substring(lines[lineCount].indexOf("#"));
				alpha = Double.parseDouble(features.substring(0,
						features.indexOf(" ")));
				feat = features.substring(features.indexOf(" ") + 1);

				for (String p : feat.split(" ")) {
					featValPair = p.split(":");
					index = Integer.parseInt(featValPair[0]);
				...
				for (String p : feat.split(" ")) {
					...
					index = Integer.parseInt(featValPair[0]);
					w.put(index,
							w.get(index) + Double.parseDouble(featValPair[1])
									* alpha);
				}
			}

			featureWeight = new FeatureWeight[w.size()];
			index = 0;
			for (Integer featureIndex : w.keySet()) {
				featureWeight[index++] = new FeatureWeight((int) featureIndex,
						w.get(featureIndex));
			}
			w.clear();

			if (!lines[5].split(" ")[1].equals("1")) {
				for (int i = 0; i < featureWeight.length; i++) {
					featureWeight[i].weight *= -1;
				}
			}
